...
export function LetterPage() {
  ...
  const loadLetter = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await fetchLetter(Number(id));
      setLetter(data);
    } catch (err) {
  ...
  const handleShare = async () => {
    ...
    const shareData = {
      title: letter?.title || 'Letter Library',
      text: letter?.description || 'Check out this beautiful letter template',
      url,
    };
  ...
  return (
    <article className="letter-page">
      ...
      <div className="letter-page__actions">
        ...
        <div className="letter-page__action-buttons">
          <button
            ...
            aria-label={isFavorite(letter.id) ? 'Remove from favorites' : 'Add to favorites'}
          >
      ...
      <header className="letter-page__header">
        ...
        <p className="letter-page__meta">{readingTime} min read</p>
      </header>
